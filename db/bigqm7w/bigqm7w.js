...
async function show(row){
  ...
  const xyzPath=`bigqm7wdef2tzvp/${row.filename}`;
  ...
  figure(el,`bigqm7wdef2tzvp_svg/${stem}.svg`,'2D structure');
  figure(el,`bigqm7wdef2tzvp_vib/${stem}.svg`,'Vibrational spectrum');
  figure(el,`bigqm7wdef2tzvp_ele/${stem}.svg`,'Electronic spectrum');
  try{
...
async function start(){setup();try{
  const response=await fetch('bigqm7wdef2tzvp.tsv');if(!response.ok)throw Error(`TSV HTTP ${response.status}`);
